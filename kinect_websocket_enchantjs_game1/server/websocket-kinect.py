...
class SendClientFactory(WebSocketClientFactory):
  ...
  def broadcast(self, msg, binary):
    ...
    if self.protocolInstance == None:
      return
    self.protocolInstance.sendMessage(msg)
...
class Kinect:
  ...
  def depthCallback(self, dev, depth, timestamp):
    ...
    if self.prev_qtl:
      radius = 20
      diff_x = str((int)(qtl)*radius - self.prev_qtl*radius)
      print diff_x
      reactor.callFromThread(self.wsFactory.broadcast, diff_x)
    
    # setup for next frame
    self.lastDepth = depth
    self.currentFrame += 1
    self.currentFrame %= self.keyFrameEvery
    self.prev_qtl = (int)(qtl)
